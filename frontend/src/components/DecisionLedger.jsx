...
export default function DecisionLedger({ accord, prompt, chatLog, onReset }) {
  ...
  const handleDownload = () => {
    ...
    const summaryDocument = [
      '========================================================================',
      '                     AGENT ACCORD - EXECUTIVE DECISION LEDGER           ',
      '========================================================================',
      '',
      `Title: ${accord.title}`,
      `Timestamp: ${new Date().toLocaleString()}`,
      `Verification Hash: ACCORD-${Math.random().toString(36).substring(2, 9).toUpperCase()}`,
      '',
      '--- 1. GOVERNANCE SCENARIO MANDATE ---',
      prompt || 'N/A',
      '',
      '--- 2. FINAL POLICY ACCORD SUMMARY ---',
      accord.summary,
      '',
      '--- 3. NUMPY RESILIENCE & ETHICS METRICS ---',
      `- Predictive Market Resilience Score: ${accord.resilience_score} / 10.0`,
      `- Governance & Layoff Fairness Score: ${accord.fairness_score} / 10.0`,
      '',
      '--- 4. MULTI-AGENT NEGOTIATION TRANSCRIPT ---',
      turns || 'No dialogue captured.',
      '',
      '========================================================================',
      ' End of Auditable Ledger Document - Generated by Agent Accord Platform ',
      '========================================================================'
    ].join('\n')

    const blob = new Blob([summaryDocument], { type: 'text/markdown;charset=utf-8' })
    const url = URL.createObjectURL(blob)
    const anchor = document.createElement('a')
    const safeTitle = accord.title.replace(/[^a-z0-9]+/gi, '-').replace(/^-+|-+$/g, '').toLowerCase()

    anchor.href = url
    anchor.download = `${safeTitle || 'agent-accord-ledger'}.md`
    anchor.click()
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between bg-slate-950/70 p-4 rounded-xl border border-slate-800">
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-xs font-semibold text-slate-200 rounded-lg border border-slate-700 flex items-center gap-1.5 transition-colors"
                >
        ...
        <div className="border-t border-slate-800/80 bg-slate-950/80 px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-3 relative z-10">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            <button
              type="button"
              onClick={handleDownload}
              className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-slate-700 bg-slate-900 text-xs font-semibold text-slate-200 transition-colors hover:border-amber-500/40 hover:bg-slate-800"
            >
              <Download className="w-4 h-4 text-amber-400" /> Download Accord (.md)
            </button>

            <button
              type="button"
              onClick={onReset}
              className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-linear-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 text-xs font-bold transition-all shadow-lg shadow-amber-500/20"
            >
